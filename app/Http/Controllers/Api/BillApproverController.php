...
class BillApproverController extends Controller
{
        ...
        try {
            ...
            $query->where(function ($q) use ($userId, $status) {
                ...
                $q->orWhere(function ($l2) use ($userId, $status) {
                    $l2
                        ->whereRaw("FIND_IN_SET('$userId', REPLACE(l2_approved_by,' ','')) > 0")
                ...
                $q->orWhere(function ($l3) use ($userId, $status) {
                    $l3
                        ->whereRaw("FIND_IN_SET('$userId', REPLACE(l3_approved_by,' ','')) > 0")
            ...
            if ($fromDate) {
                $query->whereRaw("DATE({$table}.punched_date) >= ?", [$fromDate]);
            }
            if ($toDate) {
                $query->whereRaw("DATE({$table}.punched_date) <= ?", [$toDate]);
            }
            if ($docType) {
                $query->where("{$table}.doc_type_id", $docType);
            }
            if ($department) {
                $query->where("{$table}.department_id", $department);
            }
            if ($subDept) {
                $query->where("{$table}.sub_department_id", $subDept);
            }
            if ($punchedBy) {
                $query->where("{$table}.punched_by", $punchedBy);
            }
            if ($scanBy) {
                $query->where("{$table}.temp_scan_by", $scanBy);
            }
            if ($classifyBy) {
                $query->where("{$table}.classified_by", $classifyBy);
            }

            if ($search !== '') {
                $query->where(function ($q) use ($search, $table) {
                    $q
                        ->where("{$table}.document_name", 'like', "%{$search}%")
            ...
            $rows = $query
                ->orderByRaw("{$table}.punched_date DESC")
                ->skip(($page - 1) * $perPage)
                ->take($perPage)
                ->get();
    ...
    public function getFinanceRejected(Request $request)
    {
        try {
            ...
            $query->where(function ($q) use ($userId) {
                // 1) L1 approved → L2 rejected
                $q->where(function ($g) use ($userId) {
                    $g
                        ->whereRaw("FIND_IN_SET('$userId', REPLACE(l1_approved_by_id,' ','')) > 0")
                        ->where('l1_approved_status', 'Y')
                        ->where('l2_approved_status', 'R');
                });
                // 2) L2 approved → L3 rejected
                $q->orWhere(function ($g) use ($userId) {
                    $g
                        ->whereRaw("FIND_IN_SET('$userId', REPLACE(l2_approved_by_id,' ','')) > 0")
                        ->where('l2_approved_status', 'Y')
                        ->where('l3_approved_status', 'R');
                });
                // 3) Finance punch rejected — user is last approver (L1, L2, or L3)
                $q->orWhere(function ($g) use ($userId) {
                    $g
                        ->where('finance_punch_status', 'R')
                        ->where(function ($last) use ($userId) {
                            ...
                            $last->orWhere(function ($l) use ($userId) {
                                $l
                                    ->whereRaw("FIND_IN_SET('$userId', REPLACE(l2_approved_by_id,' ','')) > 0")
    ...
    public function action(Request $request)
    {
        ...
        foreach ($levels as $i => $level) {
            $prevLevelOk = $i === 0
                ? true
                : ($bill->{$levels[$i - 1] . '_approved_status'} === 'Y');
            ...
            // Re-action allowed if finance was rejected and this is the last approver
            if ($bill->finance_punch_status === 'R' && $isMyLevel && $myLevelNum === $maxLevel) {
                $canAct = true;
            }

            // Re-action allowed if a higher level rejected
